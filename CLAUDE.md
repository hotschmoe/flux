...
Declarative UI framework combining GPU-accelerated rendering with comptime element trees.
...
- **Comptime Element Trees** - Elements are comptime-known types, no heap allocations
- **Inline Styles** - Styles as comptime structs, not runtime-parsed strings
- **Arena Allocation** - Frame-scoped arenas freed in bulk at frame end
- **SDF Text Rendering** - Crisp text at any scale via signed distance fields
- **Flexbox Layout** - Familiar layout model for positioning
...
1. Declarative views - describe what UI looks like given state
2. Retained structure - FLUX maintains element tree, diffs on updates
3. Immediate rendering - visible elements emit draw commands each frame
4. GPU-batched drawing - all primitives batched into minimal draw calls
...
Top layer of the zig-graphics stack.
